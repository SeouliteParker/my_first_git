...
**구조 선정 기준**: `Dockerfile`은 `docker build .` 실행 시 저장소 최상위에 있어야 경로 지정 없이 바로 빌드되므로 루트에 둠. 실행 코드(`app/`)와 검증용 스크린샷(`docs/images/`)은 서로 성격이 달라 분리함 — `app/`은 이미지 빌드·컨테이너 실행에 실제로 관여하는 파일이고, `docs/images/`는 결과를 증빙하기 위한 캡처 자료일 뿐 애플리케이션 동작과는 무관하기 때문. `README.md`는 저장소 진입점이라 최상위에 유지.
...
- **이미지 vs 컨테이너 (빌드/실행/변경 관점)**: 이미지(`my-node-app:1.0`)는 `docker build` 시점에 `Dockerfile`의 각 명령이 레이어로 쌓여 만들어지는 **읽기 전용 템플릿**이고, 컨테이너(`my-running-app`, `web-app`, `bind-test`, `volume-test` 등)는 그 이미지를 `docker run`으로 **실행한 개별 인스턴스**임. 같은 이미지로 여러 컨테이너를 동시에 띄워도(실제로 위에서 여러 개 띄움) 서로 독립적으로 동작하고, 컨테이너 안에서 파일을 바꾸거나 프로세스를 실행해도 그 변경은 해당 컨테이너의 쓰기 가능한 레이어에만 남을 뿐 원본 이미지(`my-node-app:1.0`)에는 반영되지 않음. 그래서 코드 자체를 바꾸려면 컨테이너를 수정하는 게 아니라 이미지를 다시 **빌드**해야 함 — "빌드는 한 번, 실행은 여러 번, 실행 중 변경은 그 컨테이너에만 국한"이 핵심 구조.
- **포트 매핑이 필요한 이유**: 컨테이너는 기본적으로 호스트와 분리된 자체 네트워크 네임스페이스를 가져서, 컨테이너 안에서 3000번 포트로 서버가 떠 있어도 호스트(내 macOS)에서는 그 포트가 보이지 않음. `-p <host>:<container>`로 호스트 포트와 컨테이너 포트를 연결해야 비로소 `curl http://localhost:3000`처럼 호스트에서 접속할 수 있음 — 실제로 위 로그·스크린샷 2장 모두 이 매핑이 있었기 때문에 접속에 성공한 것이고, 포트 충돌 트러블슈팅([9번-7](#9-트러블슈팅))에서도 매핑된 호스트 포트가 겹치면 아예 컨테이너 실행 자체가 거부된다는 걸 확인함.
- **포트 설정 재현 방법**: `Dockerfile`의 `EXPOSE 3000`은 컨테이너 내부 포트를 고정하고, 호스트 쪽 포트는 `docker run` 실행 시점에 `-p <원하는 호스트 포트>:3000`으로 자유롭게 바꿀 수 있음. 이 저장소를 그대로 재현하려면:
...
호스트 3000번 포트가 이미 사용 중이면 `-p` 앞의 숫자만 다른 값(예: `8080:3000`)으로 바꿔 재시도하면 됨 — 실제로 위 트러블슈팅에서 이 방식으로 해결함.
...
**데이터 유실 방지**: 이 미션에서는 처음부터 `/app/data`를 볼륨(`my-app-volume`)에 연결해뒀기 때문에 실제로 데이터가 사라지는 상황은 겪지 않음. 만약 볼륨이나 바인드 마운트 없이 컨테이너 내부 파일시스템에만 데이터를 저장했다면, `docker rm`으로 컨테이너를 삭제하는 순간 그 컨테이너의 쓰기 가능 레이어도 함께 삭제되어 데이터가 그대로 사라짐 — 위 `volume-test` 삭제 후 `volume-test-2`로 데이터가 유지되는 걸 확인한 테스트 자체가, 컨테이너 생명주기와 데이터를 분리해두면 이런 유실을 막을 수 있다는 걸 사전에 검증해 둔 것.

**볼륨 설정 재현 방법**: 아래 순서 그대로 실행하면 이 저장소의 볼륨 영속성 테스트를 동일하게 재현할 수 있음:

```bash
$ docker volume create my-app-volume
$ docker run -d --name my-app -v my-app-volume:/app/data -p 3000:3000 my-node-app:1.0
$ docker exec my-app sh -c 'echo "test" > /app/data/test.txt'
$ docker rm -f my-app
$ docker run -d --name my-app-2 -v my-app-volume:/app/data -p 3000:3000 my-node-app:1.0
$ docker exec my-app-2 cat /app/data/test.txt   # 삭제 전 값이 그대로 나오면 재현 성공
```
...
### 7) 포트 매핑 시 "port is already allocated"
- **문제**: `docker run -d --name web-app -p 3000:3000 my-node-app:1.0` 실행 시 `Error response from daemon: ... Bind for 0.0.0.0:3000 failed: port is already allocated`
- **원인 가설**: 바로 앞서 `docker run -d -p 3000:3000 --name my-running-app my-node-app:1.0`으로 띄워둔 컨테이너가 이미 호스트 3000번 포트를 점유하고 있을 것으로 추정
- **확인**: `docker ps`로 확인한 결과 `my-running-app` 컨테이너가 `0.0.0.0:3000->3000/tcp` 상태로 실행 중이었음 — 같은 호스트 포트를 두 컨테이너가 동시에 쓸 수 없다는 게 원인으로 확인됨
- **해결/대안**: 진단 순서는 ① `docker ps`로 어떤 컨테이너가 해당 포트를 점유 중인지 확인 → ② 기존 컨테이너를 유지할지 정리할지 판단 → ③ 여기서는 실패한 `web-app` 컨테이너를 `docker rm -f web-app`으로 정리하고, 호스트 포트를 `8080`으로 바꿔(`-p 8080:3000`) 재시도해 해결함. 컨테이너 내부 포트(3000)는 고정해도 호스트 포트는 비어있는 값으로 자유롭게 바꿀 수 있다는 걸 실제로 확인한 사례.
